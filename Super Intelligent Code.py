...
# ==============================================================================
# Major Improvements:
# 1. Dynamic Greeks recalculation (every cycle)
# 2. Robust risk management & stop-losses
# 3. Data validation & freshness checks
# 4. Liquidity filtering
# 5. American option pricing
# 6. Dividend adjustment
# 7. Accurate time-to-expiry calculation
# 8. Monte Carlo PoP estimation
# 9. Position tracking & trade journal
...
class IronCondor:
    ...
    def update_greeks(self, greeks_data: Dict[str, Dict[str, float]], spot: float, T: float):
        """Update Greeks for all legs"""
        for leg in self.legs:
            if leg.symbol in greeks_data:
...
def get_nifty_option_chain() -> Tuple[pd.DataFrame, datetime]:
    """Fetch and parse NIFTY option chain"""
    try:
        ...
        if len(upcoming_expiries) == 0:
            raise ValueError("No upcoming option expiries found")
...
def main():
    ...
    with st.sidebar:
        ...
        enable_backtesting = st.checkbox("Enable Backtesting Mode", value=False)
    ...
    try:
        ...
        if not spot_valid:
            st.warning(f"⚠️ {spot_err}. Using fallback spot price.")
            spot_price = 24383.60
        else:
        ...
        if df_quant is not None and not df_quant.empty:
            ...
            if result:
                st.success(f"✅ Optimal Setup Found (Score: {float(result.entry_note.split('Score: ')[1].split(' ')[0]):.4f})")
                ...
                with action_col1:
                    if st.button("✅ ENTER TRADE", key="enter_btn"):
            ...
            st.error("❌ Failed to fetch option chain data. Check API connectivity.")
